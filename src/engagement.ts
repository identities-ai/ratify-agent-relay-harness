...
// By default the run verifies the COMMITTED evidence trail and leaves it untouched, so a
// verification run never dirties a clone (the ML-DSA half of each hybrid signature is hedged,
// so regenerated bytes always differ; see README). Pass --write-evidence to regenerate evidence/.
...
async function scene5_publish_and_reverify(chain: Chain, claims: Claim[]): Promise<{ verified: number; total: number }> {
  ...
  if (WRITE_EVIDENCE) {
    ...
    console.log("scene5  evidence/ regenerated (--write-evidence)");
  }
